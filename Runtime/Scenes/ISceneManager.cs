...
namespace Ursa
{
    ...
    public interface ISceneManager
    {
        ...
        /// </summary>
        Task ResetAsync<TScene>(ISceneParameter parameter, TransitionType transitionType = TransitionType.Default) where TScene : MonoBehaviour;

        /// <summary>
        /// 現在のシーンの上に、指定したシーンを新しく重ねて（Additive）履歴に追加します。
        /// パラメーターの IsHistory が false の場合、シーンは表示されますが履歴には積まれません。
        /// </summary>
        Task PushAsync<TScene>(ISceneParameter parameter, TransitionType transitionType = TransitionType.Default) where TScene : MonoBehaviour;

        /// <summary>
        /// 履歴スタックの一番上にある最前面のシーンを破棄し、一つ前のシーンに戻ります。
        /// </summary>
        Task PopAsync(TransitionType transitionType = TransitionType.Default);
        
        /// <summary>
        /// 現在の最前面のシーンを破棄し、同じ階層に新しいシーンをロードして履歴を入れ替えます。
        /// </summary>
        Task ReplaceAsync<TScene>(ISceneParameter parameter, TransitionType transitionType = TransitionType.Default) where TScene : MonoBehaviour;
        ...
        /// </summary>
        Task<TScene> CreateSceneAsync<TScene>(ISceneParameter parameter = null, TransitionType transitionType = TransitionType.Default) where TScene : MonoBehaviour;

        /// <summary>
        /// 既にロード済みのシーンのインスタンスを、現在のシーンの上に重ねて履歴に追加します。
        /// </summary>
        Task PushInstanceAsync(UnityEngine.SceneManagement.Scene scene, TransitionType transitionType = TransitionType.Default);
        ...
        /// </summary>
        Task JumpToAsync<TScene>(TransitionType transitionType = TransitionType.Default) where TScene : MonoBehaviour;

        /// <summary>
        /// 指定したインデックスのシーンまで一気にPopします（インデックス0が最も古い）。
        /// 範囲外の場合は ArgumentOutOfRangeException をスローします。
        /// </summary>
        Task JumpToIndexAsync(int index, TransitionType transitionType = TransitionType.Default);
    }
